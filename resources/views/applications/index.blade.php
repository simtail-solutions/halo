...
<form class="w-100" action="{{ route('applications.index') }}" method="GET" role="search">
                ...
                <div class="mx-auto p-2" data-toggle="tooltip" title="Include spaces when search Phone Numbers #### ### ###" data-placement="right" ><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="#9DD8C3" class="bi bi-question-circle-fill" viewBox="0 0 16 16">
...
<div id="filter-buttons" class="d-flex justify-content-between my-3">
    <a href="{{ asset('/applications/?') }}"  class="all-apps {{ (request()->query('term')) == null ? 'active' : '' }}">All Applications</a>
    <a href="{{ asset('/applications/?term=1') }}" class="incc {{ (request()->query('term')) == '1' ? 'active' : '' }}">Incomplete</a>
    <a href="{{ asset('/applications/?term=2') }}" class="sub {{ (request()->query('term')) == '2' ? 'active' : '' }}">Submitted</a>
    <a href="{{ asset('/applications/?term=3') }}"  class="declined {{ (request()->query('term')) == '3' ? 'active' : '' }}">Declined</a>
    <a href="{{ asset('/applications/?term=4') }}"  class="wthdrwn {{ (request()->query('term')) == '4' ? 'active' : '' }}">Withdrawn</a>
    <a href="{{ asset('/applications/?term=5') }}"  class="aproved {{ (request()->query('term')) == '5' ? 'active' : '' }}">Approved</a>         
...
@if(auth()->user()->isAdmin() or auth()->user()->id == $application->user_id)
    ...
    <tr class="m-3 {{ isset($application->name) ? $application->name : '' }}">
        ...
        @if(auth()->user()->isAdmin())
        <td><a href="{{ route('users.show', $application->user_id) }}">{{ $application->businessName }}</a></td>
        <td class="d-flex justify-content-end">
    @if(URL::current() !== route('archived-applications.index')) 
        <a href="{{ route('application.show', $application->api_token) }}" class="applications-open px-2" alt="View application" title="View Application"><svg xmlns="http://www.w3.org/2000/svg" class="bi bi-file-earmark-plus-fill" viewBox="0 0 16 16" width="24" height="24" fill="#9DD8C3">
...
@if(URL::current() == route('archived-applications.index')) 
<a href="{{route('applications.restore', $application->id) }}" alt="Restore application" title="Restore" class="px-2"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="#9dd8c3" class="bi bi-reply-fill" viewBox="0 0 16 16">
...
<form action="{{ route('applications.destroy', $application->id) }}" method="POST">
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="#e98787" class="bi bi-file-earmark-minus-fill" viewBox="0 0 16 16">
  <path d="M9.293 0H4a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V4.707A1 1 0 0 0 13.707 4L10 .293A1 1 0 0 0 9.293 0zM9.5 3.5v-2l3 3h-2a1 1 0 0 1-1-1zM6 8.5h4a.5.5 0 0 1 0 1H6a.5.5 0 0 1 0-1z"></path>
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="#e98787" class="bi bi-x-circle-fill" viewBox="0 0 16 16">
